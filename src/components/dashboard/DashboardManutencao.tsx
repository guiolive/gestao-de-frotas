/**
 * Visão de Manutenção: foco em OS, preventivas e cumprimento de prazo.
 * É o dashboard "antigo" — o que existia antes da segmentação por setor.
 * Server component; recebe os dados pré-buscados via props para deixar
 * o `page.tsx` (dispatch) lendo só o usuário e o setor.
 */
import Link from "next/link";
import StatusBadge from "@/components/StatusBadge";
import type { Veiculo, Manutencao, AlertaKm, Agendamento } from "@/generated/prisma/client";

type ManutencaoComRel = Manutencao & {
  veiculo: Veiculo;
  itens: { valor: number }[];
};
type AlertaComRel = AlertaKm & { veiculo: Veiculo };

type ManutencaoCalculada = ManutencaoComRel & {
...
type AlertaCalculado = AlertaComRel & {
  kmAtual: number;
...
export type DashboardManutencaoProps = {
  totalVeiculos: number;
  veiculosDisponiveis: number;
  veiculosEmUso: number;
  veiculosManutencao: number;
  veiculosInativos: number;
  manutencoesComSemaforo: ManutencaoCalculada[];
  manutencoesAtrasadas: ManutencaoCalculada[];
  alertasCriticos: AlertaCalculado[];
  alertasProximos: AlertaCalculado[];
  alertasUrgentes: AlertaCalculado[];
  proximosAgendamentos: (Agendamento & { veiculo: Veiculo })[];
  mttr: number;
  pctPreventiva: number;
  pctPrazo: number;
  totalRecentes: number;
  preventivas: number;
  comPrazoLength: number;
  noPrazoLength: number;
...
export default function DashboardManutencao(props: DashboardManutencaoProps) {
  const {
    totalVeiculos,
    veiculosDisponiveis,
    veiculosEmUso,
  ...
  return (
    <>
      {/* Alertas urgentes */}
      {(alertasCriticos.length > 0 || manutencoesAtrasadas.length > 0) && (
              ...
              <div className="space-y-1">
                {alertasCriticos.map((a) => (
                  ...
                  >
                    ...
                    (KM atual: {a.kmAtual.toLocaleString("pt-BR")}, troca em:{" "}
          ...
          <div className="space-y-1">
            {alertasProximos.slice(0, 5).map((a) => (
              ...
              >
                ...
                {a.kmMedioDia > 0 && (
                  <span className="text-gray-400 ml-1">• {a.kmMedioDia} km/dia</span>
                )}
              </Link>
            ))}
          </div>
            ...
            <table className="min-w-full">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-4 py-2 text-center text-xs font-medium text-gray-500 uppercase">Dias na Oficina</th>
                  <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Previsão Saída</th>
                  ...
                  <tr key={m.id} className="hover:bg-gray-50">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <span className={`inline-block w-3 h-3 rounded-full ${semaforoDot(m.diasRestantes)}`} />
                        {m.diasRestantes < 0 ? (
                    ...
                    <td className="px-4 py-3">
                      <Link href={`/veiculos/${m.veiculoId}`} className="font-semibold text-sm text-gray-900 hover:text-blue-600">
                        {m.veiculo.placa}
                      </Link>
                    ...
                    <td className="px-4 py-3 text-center">
                      <span className={`inline-block px-2 py-1 rounded text-xs font-semibold border ${semaforoClass(m.diasRestantes)}`}>
                        {m.diasNaOficina} dia{m.diasNaOficina !== 1 ? "s" : ""}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-sm text-gray-700">
                      {m.previsaoSaida ? formatDate(m.previsaoSaida) : "Não definida"}
                    </td>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div className="bg-white rounded-lg shadow p-5">
          <p className="text-xs text-gray-500 uppercase tracking-wide mb-1">Disponibilidade</p>
          <div className="flex items-end gap-2">
            <p className={`text-3xl font-bold ${disponibilidade >= 85 ? "text-green-700" : disponibilidade >= 70 ? "text-yellow-700" : "text-red-700"}`}>
              {disponibilidade}%
            </p>
          ...
          <div className="w-full bg-gray-100 rounded-full h-2 mt-2">
            <div
              className={`h-2 rounded-full ${disponibilidade >= 85 ? "bg-green-500" : disponibilidade >= 70 ? "bg-yellow-500" : "bg-red-500"}`}
              style={{ width: `${disponibilidade}%` }}
            />
          </div>
        </div>
        <div className="bg-white rounded-lg shadow p-5">
          <p className="text-xs text-gray-500 uppercase tracking-wide mb-1">MTTR</p>
          <p className="text-3xl font-bold text-gray-900">{mttr > 0 ? mttr : "—"}</p>
          <p className="text-xs text-gray-400">dias médios na oficina (6 meses)</p>
        </div>
        <div className="bg-white rounded-lg shadow p-5">
          <p className="text-xs text-gray-500 uppercase tracking-wide mb-1">Preventiva vs Corretiva</p>
          <div className="flex items-end gap-2">
            <p className={`text-3xl font-bold ${pctPreventiva >= 70 ? "text-green-700" : pctPreventiva >= 50 ? "text-yellow-700" : "text-red-700"}`}>
          ...
          {totalRecentes > 0 && (
            <div className="flex gap-1 mt-2">
              <div className="h-2 rounded-l-full bg-green-500" style={{ width: `${pctPreventiva}%` }} />
              <div className="h-2 rounded-r-full bg-orange-400" style={{ width: `${100 - pctPreventiva}%` }} />
            </div>
          )}
          {totalRecentes > 0 && (
        ...
        <div className="bg-white rounded-lg shadow p-5">
          <p className="text-xs text-gray-500 uppercase tracking-wide mb-1">Prazo Cumprido</p>
          <p className={`text-3xl font-bold ${pctPrazo >= 80 ? "text-green-700" : pctPrazo >= 60 ? "text-yellow-700" : "text-red-700"}`}>
            {comPrazoLength > 0 ? `${pctPrazo}%` : "—"}
          </p>
          <p className="text-xs text-gray-400">
            {comPrazoLength > 0 ? `${noPrazoLength} de ${comPrazoLength} no prazo (6 meses)` : "sem dados"}
          </p>
        </div>
      </div>

      {/* Projeção de preventivas */}
      {alertasUrgentes.filter((a) => a.dataEstimada && a.kmRestante > 0).length > 0 && (
        <div className="bg-white rounded-lg shadow mb-6">
            ...
            <table className="min-w-full">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Veículo</th>
                  <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Tipo</th>
                  <th className="px-4 py-2 text-right text-xs font-medium text-gray-500 uppercase">KM Atual</th>
                  <th className="px-4 py-2 text-right text-xs font-medium text-gray-500 uppercase">Próxima Troca</th>
                  <th className="px-4 py-2 text-right text-xs font-medium text-gray-500 uppercase">KM Restante</th>
                  <th className="px-4 py-2 text-right text-xs font-medium text-gray-500 uppercase">KM/dia</th>
                  <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Data Estimada</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {alertasUrgentes
                  ...
                  .map((a) => {
                    ...
                    return (
                      <tr key={a.id} className="hover:bg-gray-50">
                        <td className="px-4 py-3">
                          <Link href={`/veiculos/${a.veiculoId}`} className="font-semibold text-sm text-gray-900 hover:text-blue-600">
                            {a.veiculo.placa}
                          </Link>
                          <p className="text-xs text-gray-400">{a.veiculo.modelo}</p>
                        </td>
                        ...
                        <td className="px-4 py-3 text-sm text-right font-semibold">
                          <span className={a.kmRestante <= a.alertaAntesDe ? "text-yellow-600" : "text-gray-900"}>
                            {a.kmRestante.toLocaleString("pt-BR")}
                          </span>
                        </td>
                        <td className="px-4 py-3 text-sm text-right text-gray-500">{a.kmMedioDia}</td>
                        <td className="px-4 py-3 text-sm">
                          <span className={`inline-block px-2 py-1 rounded text-xs font-semibold border ${semaforoClass(diasAte)}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="divide-y">
            {proximosAgendamentos.length === 0 ? (
              ...
              proximosAgendamentos.map((a) => {
                const diasAte = diasEntre(new Date(), new Date(a.dataInicio));
                return (
                  <div key={a.id} className="px-6 py-3 flex justify-between items-center hover:bg-gray-50">
                    <div>
                      <p className="font-medium text-gray-900">{a.veiculo.placa}</p>
                      <p className="text-sm text-gray-500">{a.solicitante} — {a.motivo}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-sm text-gray-700">{formatDate(a.dataInicio)}</p>
                      <span className={`inline-block px-2 py-0.5 rounded text-xs font-semibold border ${semaforoClass(diasAte)}`}>
                        {diasAte < 0 ? `${Math.abs(diasAte)}d atrasado` : diasAte === 0 ? "Hoje" : diasAte === 1 ? "Amanhã" : `${diasAte} dias`}
                      </span>
                    </div>
                  </div>
                );
        ...
        <div className="bg-white rounded-lg shadow p-6">
          <h2 className="text-lg font-semibold text-gray-800 mb-4">Resumo do Departamento</h2>
